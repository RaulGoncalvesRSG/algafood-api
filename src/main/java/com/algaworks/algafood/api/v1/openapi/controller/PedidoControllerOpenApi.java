...
public interface PedidoControllerOpenApi {
	...
	@PageableParameter
	ResponseEntity<PagedModel<PedidoResumoDTO>> pesquisar(@Parameter(hidden = true) PedidoFilter filter, @Parameter(hidden = true) Pageable pageable);

	@Operation(summary = "Registra um pedido", responses = {
			@ApiResponse(responseCode = "201", description = "Pedido registrado"),
	})
	ResponseEntity<PedidoDTO> adicionar(@RequestBody(description = "Representação de um novo pedido", required = true) PedidoRequestDTO requestDTO);
	...
	ResponseEntity<PedidoDTO> buscar(@Parameter(description = "Código de um pedido", example = "04813f77-79b5-11ec-9a17-0242ac1b0002", required = true) String codigoPedido);
